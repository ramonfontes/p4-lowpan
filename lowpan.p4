...
parser MyParser(packet_in packet,
    ...
    state parse_icmpv6_dao {
        packet.extract(hdr.icmpv6_dao);
        meta.meta.custom_value = hdr.icmpv6_dao.code;
        transition accept;
    }
...
control MyIngress(inout headers hdr,
    ...
    action set_egress_port(egressSpec_t port) {
        if (hdr.ipv6.payloadLength == 136){
            standard_metadata.egress_spec = 0;
        }
        else{
            if (meta.meta.custom_value == 0) {
                hdr.ethernet.etherType = 0x1000; //dis
            }
            else if (meta.meta.custom_value == 1) {
                hdr.ethernet.etherType = 0x1001; //dio
            }
            else if (meta.meta.custom_value == 2) {
                hdr.ethernet.etherType = 0x1002; //dao
            }
            else if (meta.meta.custom_value == 3) {
...
#        hdr.ethernet.dstAddr = 0x112233445566;  // Exemplo de endereço MAC de destino
 #       hdr.ethernet.srcAddr = 0x665544332211;  // Exemplo de endereço MAC de origem
        port_table.apply();
  #      hdr.lowpan.setValid();
    }
}
